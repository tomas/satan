...
var build_install = function(distro, opts, cb) {
  ...
  builder.generate(opts, template_path, extension, function(err, temp_script) {
    ...
    commands.install_script(distro, opts.key, temp_script, function(err) {
      if (err) return cb(err);

      load_config(opts.key, cb);
    });
  });
}

var load_config = function(key, cb) {
  var cmd = init_system == 'upstart' ? 'initctl reload-configuration' : 'systemctl daemon-reload';
  exec(cmd + ' ' + key, cb);
}
...
exports.destroy = function(key, cb) {
  ...
  setup_distro(function(err, distro) {
    ...
    self.exists(key, function(err, exists) {
      if (err || !exists)
        return cb(new Error('Service not found.'), distro);
