...
var commands = {
  status : 'initctl --system status $1',
//  load   : 'initctl start $1',
//  unload : 'initctl stop $1',
  start  : 'initctl --system start $1',
  stop   : 'initctl --system stop $1',
  reload : 'initctl --system reload-configuration'
}
...
exports.exists = function(key, cb) {
  run('status', key, function(err, out) {
    if (err) {
      if (err.message.match(/unrecognized service|unknown job/i))
...
exports.status = function(key, cb) {
  run('status', key, function(err, stdout) {
...
exports.unload = function(key, cb) {
  run('stop', key, function(err) {
    ...
    if (err && !err.message.match(/unknown instance/i))
      return cb(err);
