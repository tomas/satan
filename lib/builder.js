...
var check = function(opts) {
  ...
  if (!opts.key || !opts.bin)
    throw new Error('Required options: key, bin');
  ...
  // start interval for LaunchDaemon
  data.start_interval  = opts.interval;
  ...
  // systemd start/monitor options
  data.sd_start_after  = opts.sd_start_after || 'network.target';
  data.sd_restart      = opts.sd_restart     || 'always';
  data.sd_kill_mode    = opts.sd_kill_mode; // can be 'process', 'mixed', 'none' or 'control-group' (default)
...
exports.generate = function(data, template_path, extension, cb) {
  ...
  var dest = join(temp_path(data.key + (extension || '')));
